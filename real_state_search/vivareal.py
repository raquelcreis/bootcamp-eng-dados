#%%
# Imports
#from os import WCONTINUED,link,setpgid
...
df = pd.DataFrame(columns=['description','address','area','bedrooms','suites','wc','parking_spots','value','condo_fee','df_tax','aptlink'])
...
while len(json_tree) > 0:
    ...
    for i in range(0,qty):
        ...
        try:
            ...
            description = '-'
        try:
            address = json_apart['address']['point']['street'] + json_apart['address']['point']['neighborhood']
        except:
            address = '-'
        try:
            area = json_apart['totalAreas']
        except:
            area = '-'
        try:
            suites = json_apart['suites']
        except:
            suites = '-'
        try:
            wc = json_apart['bathrooms']
        except:
            wc = '-'
        try:
            parking_spots = json_apart['parkingSpaces']
        except:
            parking_spots = '-'
        try:
        ...
        df.loc[df.shape[0]] = [description,address,area,suites,wc,parking_spots,value,condo_fee,df_tax,aptlink]
    # Next Request
